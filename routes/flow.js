...
// ============================================================================
// NEW ENDPOINTS - ADDED FOR DATABASE STATS WHEN MARKET CLOSED
// ============================================================================

/**
 * GET /api/flow/stats/db/:ticker
 * Get aggregate stats from database for a ticker (works when market closed)
 * This is a NEW endpoint that doesn't conflict with existing /stats/ticker/:ticker
 */
router.get('/stats/db/:ticker', async (req, res) => {
  try {
    ...
    // Get stats from database (today's trades)
    const statsQuery = `
      SELECT 
        COUNT(*) as total_trades,
    ...
    const result = await pool.query(statsQuery, [ticker.toUpperCase(), today]);
    const stats = result.rows[0];
    
    // Calculate ratios
    const callPutRatio = stats.put_premium > 0 
      ? (stats.call_premium / stats.put_premium).toFixed(2)
      : 'N/A';
    ...
    res.json({
      ...
      stats: {
        ...
        calls: {
          ...
          percentage: stats.total_premium > 0 
 ...
 * This removes trades older than today to keep database clean
 */
router.post('/reset', async (req, res) => {
  try {
    console.log('🔄 Resetting flow data - deleting old trades...');
    
    // Delete trades older than today
    const deleteQuery = `
